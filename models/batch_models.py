...
class BatchJob(db.Model):
    ...
    # Configuration
    languages = Column(JSON, nullable=False)  # ["en", "es", "fr"]
...
class BatchLot(db.Model):
    ...
    image_urls = Column(JSON, nullable=False)  # [{"url": "..."}, ...]
    
    # Processing status
    status = Column(String(20), nullable=False, default='pending')  # pending, processing, completed, failed
    
    # Results
    vision_result = Column(Text, nullable=True)  # Raw vision API result
    translations = Column(JSON, nullable=True)  # {"en": "text", "es": "text"}
    
    # Error handling
    error_message = Column(Text, nullable=True)
    missing_images = Column(JSON, nullable=True)  # URLs that failed validation
...
class BatchResult(db.Model):
    ...
    # Metadata
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    file_size = Column(Integer, nullable=True)  # Size in bytes for caching
